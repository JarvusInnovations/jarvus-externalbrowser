...
Ext.define('Jarvus.util.ExternalBrowser', {
    ...
    open: function(url) {
        if (!this.isHomescreenApp()) {
            window.open(url, '_blank', 'location=no,enableViewportScale=yes,toolbarposition=top');
            return;
        }
    ...
    setupLinkHandler: function() {
        if (!this.isHomescreenApp()) {
            ...
            document.body.addEventListener('click', function(e) {
                ...
                if (target) {
                    e.preventDefault();
                    window.open(target.href, target.target, 'location=no,enableViewportScale=yes,toolbarposition=top');
                }
            }, false);
        }
    },
